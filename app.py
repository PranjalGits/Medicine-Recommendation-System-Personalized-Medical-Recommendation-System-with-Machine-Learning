...
st.write("Enter your symptoms separated by commas (e.g. headache, nausea, fever):")
user_input = st.text_input("Symptoms", placeholder="Type symptoms here...")

st.subheader("🎤 Or Speak Your Symptoms")
...
symptom_input = st.text_input("📝 Enter your symptoms (comma-separated)", 
    value=st.session_state.get("spoken_symptoms", ""))

if st.button("Predict Disease"):
    if not symptom_input or symptom_input.lower() == "symptoms":
        ...
        st.markdown("### 🏋️ Recommended Workout")
...
st.subheader("📑 Upload Medical Report for Analysis")
uploaded_file = st.file_uploader("Upload a medical report (.txt)", type="txt")

if uploaded_file is not None:
    report_text = uploaded_file.read().decode("utf-8")

    if st.button("Analyze Report with Multidisciplinary Team"):
        with st.spinner("Analyzing medical report..."):
            ...
            st.markdown("### 🧠 Final Diagnosis by Multidisciplinary Team")
...
# Voice and Vision chatbot
st.subheader("🧠👂 Voice and Vision Medical Chatbot")
audio_file = st.file_uploader("🎤 Upload an audio file (MP3/WAV)", type=["mp3", "wav"])
image_file = st.file_uploader("🖼 Upload a face/skin image", type=["jpg", "jpeg", "png"])
    ...
    if not audio_file:
        ...
        if image_file:
            ...
            doctor_response = analyze_image_with_query(
                query=f"Based on this image and the patient saying: {transcription}",
                model="meta-llama/llama-4-scout-17b-16e-instruct",
                encoded_image=encoded_img
            )
        else:
            doctor_response = f"Based on the audio input: {transcription}"

        st.markdown("### 🧑‍⚕️ Doctor's Diagnosis")
        st.write(doctor_response)

        audio_output_path = os.path.join("results", "doctor_voice_response.mp3")
...
# Chat with AI Doctor
st.subheader("💬 Chat with Your AI Doctor")
...
if user_input := st.chat_input("Ask me anything..."):
    ...
    output_voice_path = "results/chatbot_voice.mp3"
    text_to_speech_with_gtts(bot_reply, output_voice_path)
    st.audio(output_voice_path, format="audio/mp3")

# Info Sections
with st.expander("ℹ️ About"):
    st.write("This app is designed to help users get a personalized medical recommendation based on their symptoms using Machine Learning and AI models.")

with st.expander("📬 Contact"):
    st.write("For queries, contact: contact@healthapp.com")

with st.expander("👨‍💻 Developer"):
    st.write("Developed by: [Your Name or Team Name]")
